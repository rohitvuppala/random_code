...
real(8),parameter :: Runiv=8.31434d0,M1=32d0,M2=16d0,TB1=1d0,TB2=3d0,Cf=2.75e16,eta=-1d0, &
...
!!!! a = [A,B,C,D,E,F,G], fpr Enthalpy/Runiv calculation, divide by molecular weight for the constants
...
rho1=rhoi
rho2=rhoi-rho1
hi =  (A1h(1)*Ti+A1h(2)*(Ti**2)/2 + A1h(3)*(Ti**3)/3+A1h(4)*(Ti**4)/4+ &
A1h(5)*(Ti**5)/5+A1h(6))
...
gr= (A(1)*(1-log(T))*T-A(7)*T+A(2)*T**2/2+A(3)*T**3/6+A(4)*T**4/12+ &
          A(5)*T**5/20+A(6))
gp = (B(1)*(1-log(T))*T-B(7)*T+B(2)*T**2/2+B(3)*T**3/6+B(4)*T**4/12+ &
          B(5)*T**5/20+B(6))*2
...
dengdT = (rho1*( (A(1)+A(2)*(T) + A(3)*(T**2)+A(4)*(T**3)+ &
       ...
       B(5)*(T**4) - 1/M2))                                          !!!!!!doubtful

T = T - deng/dengdT


print*, n,time,T
if (abs(deng/dengdt) < 10e-8) then
